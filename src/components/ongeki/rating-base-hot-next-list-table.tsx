...
const RatingBaseHotNextListTable = ({
	songs,
	searchQuery,
	onSearchChange,
}: RatingBaseHotNextListTableProps) => {
	return (
					...
					<TableBody>
						{songs.map((song, index) => (
							<TableRow key={song.id ?? index} className="hover:bg-gray-700 border-b border-gray-700">
								...
								<TableCell className="text-sm text-gray-300">
									{song.type === "userRatingBaseHotNextList" ? "Hot Next" : "Next"}
								</TableCell>
							</TableRow>
						))}
					</TableBody>
...
export default RatingBaseHotNextListTable;
